...
class MarketGenerator:
    ...
    def generate_agents(self):
        ...
        for i in range(self.broker_args["num_brokers"]):
            self.brokers.append(Broker(str(i), self.broker_args, self.num_risk_models, self.sim_args, self.risk_model_configs))

        # Generate syndicates
        for i in range(self.syndicate_args["num_syndicates"]):
            self.syndicates.append(Syndicate(str(i), self.syndicate_args, self.num_risk_models, self.sim_args, self.risk_model_configs))

        # Generate reinsurancefirms
        if self.with_reinsurance:
            for i in range(self.reinsurancefirm_args["num_reinsurancefirms"]):
                self.reinsurancefirms.append(ReinsuranceFirm(str(i), self.reinsurancefirm_args, self.num_risk_models, self.risk_model_configs))

        # Generate shareholders
        for i in range(self.shareholder_args["num_shareholders"]):
            self.shareholders.append(Shareholder(str(i), self.shareholder_args, self.num_risk_models, self.risk_model_configs))
